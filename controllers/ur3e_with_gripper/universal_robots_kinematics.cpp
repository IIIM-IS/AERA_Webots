...
namespace universalRobots
{
	...
	void UR::inverseKinematics(const pose& targetTipPose, float(*outIkSols)[m_numIkSol][m_numDoF])
	{
		...
		{
			(*outIkSols)[i + 4][0] = (M_PI / 2 + theta1_psi + (i < 0 ? 1 : -1) * theta1_phi) - M_PI; // (i < 0 ? 1 : -1) first 4 theta1 values have positive phi
		}
		...
		for (unsigned int i = 0; i < m_numIkSol; i++)
		{
		 ...
		 // There are two possible solutions for theta5, that depend on whether the wrist joint is up or down.
			...
			// Computing theta6.
			if ((*outIkSols)[i][4] == 0 || (*outIkSols)[i][4] == 2 * M_PI) // If theta5 is equal to zero.
				(*outIkSols)[i][5] = 0.0f; // Give arbitrary value to theta6
			else
			{
				const float sinTheta5 = sin((*outIkSols)[i][4]);
				(*outIkSols)[i][5] = M_PI / 2 + atan2(-T_16.inverse()(1, 1) / sinTheta5, T_16.inverse()(0, 1) / sinTheta5);
			...
			// T_45 = T_44'*T_4'5
			Eigen::Matrix4f T_44_ = mathLib::calcTransformationMatrix(Eigen::RowVector4f(0.0f, m_a[2], m_d[4], M_PI / 2));
			Eigen::Matrix4f T_4_5 = mathLib::calcTransformationMatrix(Eigen::RowVector4f(M_PI / 2, 0.0f, 0.0f, (*outIkSols)[i][4]));
			Eigen::Matrix4f T_45 = T_44_ * T_4_5;

			// T_56 = T_55'*T_5'6
			Eigen::Matrix4f T_55_ = mathLib::calcTransformationMatrix(Eigen::RowVector4f(-M_PI / 2, 0.0f, 0.0f, -M_PI / 2));
			...
			{
				// Computing theta3.
				(*outIkSols)[i][2] = M_PI -theta3_psi;
				// Masking theta3 for CoppeliaSim(invert value for ang > 180).
				// if ((*outIkSols)[i][2] > M_PI)
				//  	(*outIkSols)[i][2] = (*outIkSols)[i][2] - M_PI *2;
				// Computing theta2.
				(*outIkSols)[i][1] = M_PI / 2 - atan2(T_14(2, 3), T_14(0, 3)) + asin((m_a[1] * sin(-theta3_psi)) / P_14_xz);
				// Computing theta4.
				Eigen::Matrix4f T_12 = mathLib::calcTransformationMatrix(Eigen::RowVector4f(-M_PI / 2, 0.0f, m_d[1], (*outIkSols)[i][1] - M_PI / 2));
